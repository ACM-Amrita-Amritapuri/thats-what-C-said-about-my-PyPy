...
class Paddle(PlayComponent):
    def __init__(self,canvas,x,y):
        self.height=7 #height of paddle
        self.width=130 #width of paddle
        self.ball=None
    ...
    def move(self,dist):
        coord=self.position()
        width=self.canvas.winfo_width() #object safe from window resizing
...
class Brick(PlayComponent):
    colorArray={3:'#35233d', 2:'#735780', 1:'#bfa5c9'}
...
class Game(tk.Frame):
    def __init__(self, master):
        ...
        self.width=1000
        self.height=500
        self.canvas= tk.Canvas(self, bg='#856ff8', height=self.height, width=self.width)
        self.canvas.pack()
        self.pack()
        self.items={}
        self.ball=None
        self.paddle=Paddle(self.canvas,self.width/2,400) #here 400 indicates the height level of paddle
        self.items[self.paddle.item]=self.paddle
        for x in range(100,self.width - 100, 60):
            self.display_brick(x+20,50,1)
            self.display_brick(x+20,70,2)
    ...
    def display_ball(self):
        ...
        self.ball=Ball(self.canvas,x,390)
